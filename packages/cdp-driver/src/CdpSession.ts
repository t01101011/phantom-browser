...
export class CdpSession {
   ...
   *   caller owns the stealth consequences. No MCP tool reaches this path
   *   (raw unsafe CDP is not exposed); it is retained only for internal use.
    ...
    if (tracked && this.opts.engine === "cloakbrowser" && CLOAK_RISKY_ENABLE_DOMAINS.has(domain)) {
      throw new Error(
        `Refusing ${method} in safe mode: enabling ${domain} trips the anti-detect ` +
          ...
          `Raw CDP is disabled by default; use the convenience wrappers instead.`,
      );
    }
