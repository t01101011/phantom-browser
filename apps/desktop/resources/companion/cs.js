// Phantom Research Companion — content script on Chrome Web Store detail pages. The
// native install path is dead in CloakBrowser, so we replace the disabled "Add
// to Chrome" button with our own "Add to Phantom Research" in the same spot and hide
...
(function () {
  ...
  // Channel to Phantom Research: write the id to a DOM attribute on <html>. CloakBrowser
  ...
  function makeButton() {
    ...
    btn.type = "button";
    btn.textContent = "Add to Phantom Research";
    btn.style.cssText = [
      "display:inline-flex",
      "align-items:center",
