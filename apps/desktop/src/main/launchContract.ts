...
export const LAUNCH_CONTRACT: FieldCoverage[] = [
  ...
  },
  {
    ...
    cloakbrowser: "native-flag",
    notes:
      "WebRTC IP handling is not a FingerprintConfig field — it is a launch artifact applied when a proxy is set. CFT is fail-closed: Chromium enterprise policy `WebRtcIPHandling=disable_non_proxied_udp` must be installed and read back successfully before spawn; policy failure aborts launch. The JS WebRTC spoof/block script is injected via CDP Page.addScriptToEvaluateOnNewDocument on page + iframe targets and Runtime.evaluate on worker targets while auto-attach pauses new targets. This remains defense in depth, not a substitute for verified policy. CloakBrowser: `--fingerprint-webrtc-ip=auto` native flag",
  },
];
